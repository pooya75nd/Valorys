...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-white/5 bg-ink-deep/90 backdrop-blur-md">
              ...
              {userMenuOpen && (
                <div className="absolute right-0 top-full mt-2 w-52 bg-zinc-900 border border-white/10 rounded-xl shadow-2xl overflow-hidden z-50">
                  ...
                  <button onClick={() => signOut()}
                    className="flex items-center gap-3 px-4 py-3 text-sm text-zinc-500 hover:bg-white/5 hover:text-red-400 transition-colors w-full">
      ...
      {mobileOpen && (
        <div className="md:hidden border-t border-white/5 bg-ink-deep px-6 py-4 flex flex-col gap-4">
          {['Opportunités|/opportunites', 'Simulateur|/simulateur', 'Tarifs|/pricing'].map(item => {
            const [label, href] = item.split('|')
            return (
              <Link key={href} href={href} onClick={() => setMobileOpen(false)}
                className="text-zinc-400 hover:text-gold-400 transition-colors py-1 text-sm">
                {label}
              </Link>
            )
          })}
          {!session && (
            <Link href="/login" onClick={() => setMobileOpen(false)}
              className="mt-2 text-center py-2 px-4 bg-gold-700/20 border border-gold-700/40 text-gold-400 rounded-lg text-sm">
              Commencer gratuitement
            </Link>
          )}
        </div>
